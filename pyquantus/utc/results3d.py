...
class UtcData3D:
    ...
    def drawCmaps(self):
        ...
        self.mbfIm = np.zeros(self.utcAnalysis.ultrasoundImage.bmode.shape[:3]).astype(int)
        self.ssIm = np.zeros(self.utcAnalysis.ultrasoundImage.bmode.shape[:3]).astype(int)
        self.siIm = np.zeros(self.utcAnalysis.ultrasoundImage.bmode.shape[:3]).astype(int)
        ...
        for i, window in enumerate(self.utcAnalysis.voiWindows):
            ...
            self.mbfIm[window.corMin: window.corMax+1, window.latMin: window.latMax+1, window.axMin: window.axMax+1] = mbfColorIdx+1 #np.array(self.mbfCmap[mbfColorIdx])*255
            self.ssIm[window.corMin: window.corMax+1, window.latMin: window.latMax+1, window.axMin: window.axMax+1] = ssColorIdx+1 #np.array(self.ssCmap[ssColorIdx])*255
            self.siIm[window.corMin: window.corMax+1, window.latMin: window.latMax+1, window.axMin: window.axMax+1] = siColorIdx+1 #np.array(self.siCmap[siColorIdx])*255
    ...
    def scanConvertCmaps(self):
        ...
        self.scMbfIm, _, _ = scanConvert3dVolumeSeries(self.mbfIm, self.scParams, scale=False, interp='nearest', normalize=False)
        self.scMbfIm = self.scMbfIm.astype(int)
        self.scSiIm, _, _ = scanConvert3dVolumeSeries(self.siIm, self.scParams, scale=False, interp='nearest', normalize=False)
        self.scSiIm = self.scSiIm.astype(int)
        self.scSsIm, _, _ = scanConvert3dVolumeSeries(self.ssIm, self.scParams, scale=False, interp='nearest', normalize=False)
        self.scSsIm = self.scSsIm.astype(int)
        
        windowPoints = np.transpose(np.where(self.scMbfIm > 0))
        scMbfParamap = np.zeros(self.scMbfIm.shape + (4,), dtype=np.uint8)
        for point in windowPoints:
            mbfIdx = int(np.round(self.scMbfIm[point[0], point[1], point[2]]-1))
            scMbfParamap[point[0], point[1], point[2], :3] = (np.array(self.mbfCmap[mbfIdx])*255).astype(int)
            scMbfParamap[point[0], point[1], point[2], 3] = 255
            
        windowPoints = np.transpose(np.where(self.scSsIm > 0))
        scSsParamap = np.zeros(self.scSsIm.shape + (4,), dtype=np.uint8)
        for point in windowPoints:
            ssIdx = int(np.round(self.scSsIm[point[0], point[1], point[2]]-1))
            scSsParamap[point[0], point[1], point[2], :3] = (np.array(self.ssCmap[ssIdx])*255).astype(int)
            scSsParamap[point[0], point[1], point[2], 3] = 255
            
        windowPoints = np.transpose(np.where(self.scSiIm > 0))
        scSiParamap = np.zeros(self.scSiIm.shape + (4,), dtype=np.uint8)
        for point in windowPoints:
            siIdx = int(np.round(self.scSiIm[point[0], point[1], point[2]]-1))
            scSiParamap[point[0], point[1], point[2], :3] = (np.array(self.siCmap[siIdx])*255).astype(int)
            scSiParamap[point[0], point[1], point[2], 3] = 255
        
        self.scMbfIm = scMbfParamap
        self.scSsIm = scSsParamap
        self.scSiIm = scSiParamap
