...
class UtcVisualizations:
    ...
    def __init__(self, utcAnalysis: Any):
        ...
        self.paramaps: List[np.ndarray]; self.scParamaps: List[np.ndarray]
    ...
    def drawParamaps(self):
        ...
        minParamVals = []; maxParamVals = []; self.paramaps = []
        self.params = []
        for param in params:
            ...
            minParamVals.append(min(paramArr))
            maxParamVals.append(max(paramArr))
            ...
            for i, window in enumerate(self.utcAnalysis.roiWindows):
                paramColorIdx = int((255 / (maxParamVals[-1]-minParamVals[-1])
                                     )*(getattr(window.results, param)-minParamVals[-1])
                                    ) if minParamVals[-1] != maxParamVals[-1] else 125
