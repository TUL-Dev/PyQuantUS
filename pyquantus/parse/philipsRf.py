...
class PhilipsRfParser:
    ...
    class Rfdata:
        ...
        def __init__(self):
            ...
            self.echoData: Tuple[np.ndarray] = None
            self.dbParams: 'PhilipsRfParser.DbParams' = None
            self.echoMModeData: Tuple[np.ndarray] = None
            self.miscData: Tuple[np.ndarray] = None
            self.cwData: Tuple[np.ndarray] = None
            self.pwData: Tuple[np.ndarray] = None
            self.colorData: Tuple[np.ndarray] = None
            self.colorMModeData: Tuple[np.ndarray] = None
            self.dummyData: Tuple[np.ndarray] = None
            self.swiData: Tuple[np.ndarray] = None
            
            logging.debug("Rfdata initialization complete")
    
    def __init__(self, use_c: bool = True, multiline_output: int = 2, multiline_input: int = 32, offset_samples: int = 2256,
                 log_level: int = logging.INFO):
        """Initialize the parser with default parameters.
        
        Args:
            use_c: Flag to use C extensions for performance (default: True)
        ...
        self.multiline_output: int = multiline_output
        self.multiline_input: int = multiline_input
        self.offset_samples: int = offset_samples
        self.log_level: int = log_level
        self.disable_tqdm = log_level > logging.DEBUG
        self.rfdata: 'PhilipsRfParser.Rfdata' = None
        self.tx_beams_per_frame: int = None
        self.num_sonoct_angles: int = None
        self.num_frames: int = None
        self.multiline_factor: int = None
        self.points_per_line: int = None
        ...
        # Define multiline (ML) sort lists
    ...
    def philipsRfParser(self, filepath: str, save_numpy: bool = False) -> np.ndarray:
        """Main parsing function. Parse Philips RF data file, save as .mat file, and return shape of data.
        ...
        try:
            ...
            self.rfdata = self._parse_rf(filepath)
            ...
            data_to_save, _ = self._find_primary_data()
            ...
            self._calculate_parsing_parameters()
    ...
    def _setup_logging(self, filepath: str, save_numpy: bool) -> dict:
        """Set up logging configuration for the parsing process."""
        logger = logging.getLogger()                # Get the root logger - it may already be configured
    ...
    def _parse_rf(self, filepath: str) -> 'PhilipsRfParser.Rfdata':
        ...
        rawrfdata, _ = self._load_raw_rf_data(filepath, total_header_size_bytes)
    ...
    def _detect_file_type(self, file_obj) -> None:
        """
        Detects the type of the RF data file. Note since each element
        in the header array represents exactly one byte to read from the file, the array length 
    ...
    def _parse_file_header_and_offset(self, file_obj) -> Tuple['PhilipsRfParser.DbParams', int, str]:
        ...
        if self.has_file_header:
            ...
            # - file_header_bytes: header bytes
    ...
    def _load_raw_rf_data(self, filepath: str, total_header_size_bytes: int) -> Tuple[Any, int]:
        """Load raw RF data from file, handling Voyager and Fusion formats."""
        logging.info(f"Loading raw RF data: is_voyager={self.is_voyager}, is_fusion={self.is_fusion}")
    ...
    def _calculate_file_sizes(self, filepath: str, total_header_size: int) -> Tuple[int, int, int, int]:
    ...
    def _align_voyager_parameters(self, remaining_size_bytes: int, read_offset_bytes: int, read_size_bytes: int) -> Tuple[int, int]:
        """Align read parameters to Voyager format boundaries."""
        alignment_bytes = np.arange(0, remaining_size_bytes + 1, VOYAGER_FORMAT_BOUND)
    ...
    def _load_fusion_data(self, filepath: str, total_header_size_bytes: int, remaining_size_bytes: int, read_offset_bytes: int, read_size_bytes: int) -> Tuple[Any, int]:
        ...
        num_clumps = int(np.floor(read_size_bytes / FUSION_FORMAT_BOUND))
    ...
    def _align_fusion_parameters(self, remaining_size_bytes: int, read_offset_bytes: int, read_size_bytes: int) -> Tuple[int, int]:
        """Align read parameters to Fusion format boundaries."""
        alignment_bytes = np.arange(0, remaining_size_bytes + 1, FUSION_FORMAT_BOUND)
        offset_diff_bytes = alignment_bytes - read_offset_bytes
        read_diff_bytes = alignment_bytes - read_size_bytes
    ...
    def _get_part_a_py(self, num_clumps: int, filepath: str, offset_bytes: int) -> list:
        ...
        with open(filepath, 'rb') as fd:
            ...
            with tqdm(total=100, desc="Processing Part A", unit="%", bar_format="{l_bar}{bar}| {n_fmt}/{total_fmt} [{elapsed}<{remaining}, {rate_fmt}]", disable=self.disable_tqdm) as pbar:
    ...
    def _get_part_b_py(self, num_clumps: int, filepath: str, offset_bytes: int) -> list:
        ...
        with open(filepath, 'rb') as fd:
            ...
            with tqdm(total=100, desc="Processing Part B", unit="%", bar_format="{l_bar}{bar}| {n_fmt}/{total_fmt} [{elapsed}<{remaining}, {rate_fmt}]", disable=self.disable_tqdm) as pbar:
    ...
    def _reshape_voyager_raw_data(self, rawrfdata: Any) -> Any:
        ...
        num_clumps = np.floor(len(rawrfdata) / VOYAGER_FORMAT_BOUND)
        ...
        if len(rawrfdata) > rlimit:
            ...
            num_clump_group = int(rlimit / VOYAGER_FORMAT_BOUND)
            logging.debug(f"Chunking: {num_chunks} chunks, {num_rem_bytes} remaining bytes, {num_clump_group} clumps per chunk")
            
            reshaped_clumps = np.zeros((num_chunks + 1, 3, 12, num_clump_group))
            m = 0
            n = 0
            
            for i in range(num_chunks):
                logging.debug(f"Processing chunk {i+1}/{num_chunks}")
                reshaped_clumps[i] = np.reshape(rawrfdata[m:m + rlimit], (3, 12, num_clump_group))
                m += rlimit
                n += num_clump_group
                
            if num_rem_bytes > 0:
                logging.debug(f"Processing remaining {num_rem_bytes} bytes")
                reshaped_clumps[num_chunks] = np.reshape(rawrfdata[m:int(num_clumps * VOYAGER_FORMAT_BOUND)], (3, 12, int(num_clumps - n)))
                
            rawrfdata = np.concatenate((reshaped_clumps[:]), axis=2)
    ...
    def _parse_header_voyager(self, rawrfdata):
        ...
        header_info = PhilipsRfParser.HeaderInfoStruct()
        iHeader, numHeaders = self._find_voyager_headers(rawrfdata)
        header_info = self._initialize_header_arrays(header_info, numHeaders)

        self._process_voyager_headers(rawrfdata, iHeader, numHeaders, header_info) # Process each header
    ...
    def _process_voyager_headers(self, rawrfdata, iHeader, numHeaders, temp_headerInfo):
        ...
        for m in tqdm(range(numHeaders), desc="Processing Voyager headers", unit="header", disable=self.disable_tqdm):
            packedHeader = self._build_voyager_packed_header(rawrfdata, iHeader, m) # Build packed header string from raw data
    ...
    def _build_voyager_packed_header(self, rawrfdata, iHeader, m):
        ...
        for k in np.arange(11, 0, -1):
            ...
            for i in np.arange(2, 0, -1):
                ...
                cur_header += bin(value)
                
            # Discard first 3 bits, redundant info
            packedHeader += cur_header[3:24]
    ...
    def _parse_voyager_header_values(self, packedHeader, m, header_info):
        ...
        # Parse 8-bit fields
        header_info.RF_CaptureVersion[m] = int(packedHeader[iBit:iBit+4], 2);           iBit += 4
        header_info.Tap_Point[m] = int(packedHeader[iBit:iBit+3], 2);                   iBit += 3
        header_info.Data_Gate[m] = int(packedHeader[iBit], 2);                          iBit += 1
        header_info.Multilines_Capture[m] = int(packedHeader[iBit:iBit+4], 2);          iBit += 4
        header_info.RF_Sample_Rate[m] = int(packedHeader[iBit], 2);                     iBit += 1
        
        # Log sample rate for first header
        if m == 0:
            logging.info(f"Sample rate from first Voyager header: {header_info.RF_Sample_Rate[m]}")
        
        header_info.Steer[m] = int(packedHeader[iBit:iBit+6], 2);                       iBit += 6
        header_info.elevationPlaneOffset[m] = int(packedHeader[iBit:iBit+8], 2);        iBit += 8
        header_info.PM_Index[m] = int(packedHeader[iBit:iBit+2], 2);                    iBit += 2
        
        # Parse 16-bit fields
        header_info.Line_Index[m] = int(packedHeader[iBit:iBit+16], 2);                 iBit += 16
        header_info.Pulse_Index[m] = int(packedHeader[iBit:iBit+16], 2);                iBit += 16
        header_info.Data_Format[m] = int(packedHeader[iBit:iBit+16], 2);                iBit += 16
        header_info.Data_Type[m] = int(packedHeader[iBit:iBit+16], 2);                  iBit += 16
        header_info.Header_Tag[m] = int(packedHeader[iBit:iBit+16], 2);                 iBit += 16
        header_info.Threed_Pos[m] = int(packedHeader[iBit:iBit+16], 2);                 iBit += 16
        header_info.Mode_Info[m] = int(packedHeader[iBit:iBit+16], 2);                  iBit += 16
        
        # Parse 32-bit fields
        header_info.Frame_ID[m] = int(packedHeader[iBit:iBit+32], 2);                   iBit += 32
        header_info.CSID[m] = int(packedHeader[iBit:iBit+16], 2);                       iBit += 16
        header_info.Line_Type[m] = int(packedHeader[iBit:iBit+16], 2);                  iBit += 16
    ...
    def _process_fusion_headers(self, rawrfdata, iHeader, numHeaders, HeaderInfo):
        ...
        for m in tqdm(range(numHeaders), desc="Processing Fusion Headers", disable=self.disable_tqdm):
    ...
    def _get_filler_zeros(num: int) -> str:
        ...
        count = max(0, num - 1) # Ensure we don't create negative length strings
        result = '0' * count
        return result
    ...
    def _parse_fusion_header_values(self, packedHeader, m, HeaderInfo):
        ...
        # Parse 8-bit fields
        HeaderInfo.RF_CaptureVersion[m] = int(packedHeader[iBit:iBit+4], 2);    iBit += 4
        HeaderInfo.Tap_Point[m] = int(packedHeader[iBit:iBit+3], 2);            iBit += 3
        HeaderInfo.Data_Gate[m] = int(packedHeader[iBit], 2);                   iBit += 1
        HeaderInfo.Multilines_Capture[m] = int(packedHeader[iBit:iBit+4], 2);   iBit += 4
        iBit += 15  # Skip 15 unused bits
        HeaderInfo.RF_Sample_Rate[m] = int(packedHeader[iBit], 2);              iBit += 1
        HeaderInfo.Steer[m] = int(packedHeader[iBit:iBit+6], 2);                iBit += 6
        HeaderInfo.elevationPlaneOffset[m] = int(packedHeader[iBit:iBit+8], 2); iBit += 8
        HeaderInfo.PM_Index[m] = int(packedHeader[iBit:iBit+2], 2);             iBit += 2
        
        # Parse 16-bit fields
        HeaderInfo.Line_Index[m] = int(packedHeader[iBit:iBit+16], 2);          iBit += 16
        HeaderInfo.Pulse_Index[m] = int(packedHeader[iBit:iBit+16], 2);         iBit += 16
        HeaderInfo.Data_Format[m] = int(packedHeader[iBit:iBit+16], 2);         iBit += 16
        HeaderInfo.Data_Type[m] = int(packedHeader[iBit:iBit+16], 2);           iBit += 16
        HeaderInfo.Header_Tag[m] = int(packedHeader[iBit:iBit+16], 2);          iBit += 16
        HeaderInfo.Threed_Pos[m] = int(packedHeader[iBit:iBit+16], 2);          iBit += 16
        HeaderInfo.Mode_Info[m] = int(packedHeader[iBit:iBit+16], 2);           iBit += 16
        
        # Parse 32-bit fields
        HeaderInfo.Frame_ID[m] = int(packedHeader[iBit:iBit+32], 2);            iBit += 32
        HeaderInfo.CSID[m] = int(packedHeader[iBit:iBit+16], 2);                iBit += 16
        HeaderInfo.Line_Type[m] = int(packedHeader[iBit:iBit+16], 2);           iBit += 16
    ...
    def _parse_data_voyager(self, rawrfdata, headerInfo: 'PhilipsRfParser.HeaderInfoStruct') -> Tuple[np.ndarray, np.ndarray]:
        ...
        for m in tqdm(range(numHeaders), desc="Processing Voyager Lines", disable=self.disable_tqdm):
    ...
    def _find_echo_data_indices(self, header_info: 'PhilipsRfParser.HeaderInfoStruct') -> Tuple[np.ndarray, int]:
        ...
        if echo_count == 0 and np.any(header_info.Data_Type == 1): # if no echo data found
    ...
    def _sort_echo_data_by_tap_point(self, rfdata: 'PhilipsRfParser.Rfdata', echo_data: np.ndarray, 
        ...
        if tap_point in [0, 1]:
            ...
            rfdata.echoData = self._sort_rf(echo_data, ML_Capture, ML_Actual, CRE, self.is_voyager)
        elif tap_point == 2:
            ...
            CRE = 1
            logging.debug(f"Tap point {tap_point}: ML_Actual={ML_Actual}, CRE={CRE}")
            rfdata.echoData = self._sort_rf(echo_data, ML_Actual, ML_Actual, CRE, self.is_voyager)
    ...
    def _prune_data(self, line_data, line_header, multiline_capture):
        ...
        num_samples = pruned_data.shape[0] # Recalculate the number of samples after initial pruning
        
        # Calculate the end point for pruning by finding indices where the line header matches
        # the condition for end point
    ...
    def _initialize_rf_sort_outputs(self, depth, ML, xmit_events, CRE):
        ...
        out0 = out1 = out2 = out3 = np.array([]) # Initialize arrays with empty values
    ...
    def _fill_output_array(self, output_array, input_array, depth, k, iML_index, Stride):
        ...
        indices = np.arange(iML_index, (depth*Stride), Stride) # Create indices for strided access
    ...
    def _extract_echo_mmode_data(self, rfdata: 'PhilipsRfParser.Rfdata', header_info: 'PhilipsRfParser.HeaderInfoStruct', tap_point: int) -> 'PhilipsRfParser.Rfdata':
        ...
        if echo_mmode_count > 0:
            ...
            multiline_actual = 1; cross_resolution_enhancement = 1
    ...
    def _calculate_parsing_parameters(self) -> None:
    ...
    def _fill_data_arrays(self) -> Tuple[np.ndarray, np.ndarray]:
        ...
        for k0 in tqdm(range(self.num_frames), desc="RF data array filling", unit="frame", disable=self.disable_tqdm):
            for k1 in range(self.num_sonoct_angles):        # Read angles within frame
                for k2 in range(self.tx_beams_per_frame):   # Read beams within angle
                    ...
                    idx0 = self.offset_samples + np.arange(self.points_per_line * self.multiline_factor); idx1 = bi
                    
                    # Reshape data for multiline
                    reshaped_linedata = np.transpose(
                        np.reshape(self.rfdata.lineData[idx0, idx1],
                                 (self.multiline_factor, self.points_per_line), order='F')
                    )
                    
                    # Log first shape for debugging
                    if k0 == 0 and k1 == 0 and k2 == 0:
                        ...
                        logging.debug(f"reshaped_linedata shape: {reshaped_linedata.shape}, reshaped_linedata sample: {reshaped_linedata.ravel()[:10]}")
                    
                    # Harmonic extraction
                    if reshaped_linedata.shape[1] > 2:
                        rftemp_all_harm[:, np.arange(self.multiline_output) + (k2 * self.multiline_output)] = reshaped_linedata[:, [0, 2]]
                    else:
                        logging.warning(f"reshaped_linedata has only {reshaped_linedata.shape[1]} columns, skipping harmonic assignment")
                    
                    # Fundamental extraction
                    if reshaped_linedata.shape[1] >= 12:
                        rftemp_all_fund[:, np.arange(self.multiline_output) + (k2 * self.multiline_output)] = reshaped_linedata[:, [9, 11]]
                    elif reshaped_linedata.shape[1] >= 2:
                        if k0 == 0 and k1 == 0 and k2 == 0:
                            logging.warning(f"reshaped_linedata has only {reshaped_linedata.shape[1]} columns, using last 2 columns for fundamental")
                        rftemp_all_fund[:, np.arange(self.multiline_output) + (k2 * self.multiline_output)] = reshaped_linedata[:, [-2, -1]]
                    else:
                        logging.warning(f"reshaped_linedata has only {reshaped_linedata.shape[1]} columns, skipping fundamental assignment")
    ...
    def _save_matlab_data(self, filepath, data_to_save, rf_data_all_fund, rf_data_all_harm):
        ...
        if hasattr(self.rfdata, 'echoData') and isinstance(self.rfdata.echoData, (list, tuple)):
            for i in range(1, len(self.rfdata.echoData)):
                contents[f'echoData{i}'] = self.rfdata.echoData[i]
...
def philipsRfParser(imagePath: str, save_numpy: bool = False, use_c: bool = True, 
                    multiline_output: int = 2, multiline_input: int = 32, offset_samples: int = 2256,
                    log_level: int = logging.INFO):
    """Entrypoint for Philips RF Parser. 

    Args:
        imagePath: Path to image to parse
        save_numpy: Flag to determine if outputted data should be saved as multiple .npy files 
            or in .mat form
        use_c: Flag to use C extensions for performance (default: True)
        multiline_output: Output multiline factor (default: 2)
        multiline_input: Input multiline factor (default: 32)
    ...
    parser = PhilipsRfParser(use_c=use_c, multiline_input=multiline_input,
                            multiline_output=multiline_output, offset_samples=offset_samples,
                            log_level=log_level)
    parser.philipsRfParser(imagePath, save_numpy)
...
from pyquantus.parse.philipsRf_old import parseRF
...
def getVolume(rfPath: Path, sipNumOutBits: int = 8, DRlowerdB: int = 20, DRupperdB: int = 40):
    ...
    #save rawData to numpy file
    np.save(f"{str(rfPath.absolute())[:-3]}_rawData.npy", rawData)
...
if __name__ == "__main__":
    ...
    parser = PhilipsRfParser(log_level=LOG_LEVEL)
    parser.philipsRfParser(filepath, save_numpy=True)
    
    getVolume(Path(filepath), rawData=parser.rfdata)
