...
def get_visualization_types() -> Tuple[dict, dict]:
        ...
        if folder.is_dir() and not folder.name.startswith("_"):
            try:
                ...
                module = importlib.import_module(
                    f"pyquantus.visualizations.{folder.name}.framework"
                )
    ...
    for type_name, type_class in types.items():
        try:
            module = importlib.import_module(f'pyquantus.visualizations.{type_name}.functions')
            for name, obj in vars(module).items():
                try:
                    if callable(obj) and obj.__module__ == f'pyquantus.visualizations.{type_name}.functions':
