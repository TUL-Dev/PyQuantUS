...
def get_data_export_types() -> Tuple[dict, dict]:
        ...
        if folder.is_dir() and not folder.name.startswith("_"):
            try:
                ...
                module = importlib.import_module(
                    f"pyquantus.data_export.{folder.name}.framework"
                )
    ...
    for type_name, _ in types.items():
        try:
            module = importlib.import_module(f'pyquantus.data_export.{type_name}.functions')
            for name, obj in vars(module).items():
                try:
                    if callable(obj) and obj.__module__ == f'pyquantus.data_export.{type_name}.functions':
