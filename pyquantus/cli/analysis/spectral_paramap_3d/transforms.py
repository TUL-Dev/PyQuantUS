...
def compute_hanning_power_spec_3d(rf_data: np.ndarray, start_frequency: int, end_frequency: int, 
    ...
    Args:
        rf_data (np.ndarray): 3D RF data from the ultrasound volume (n lateral lines x m axial samples x l elevational lines).
    ...
    n_lateral, m_axial, l_elevational = rf_data.shape
    
    # Create Hanning Window Function for the axial dimension
    unrm_wind = np.hanning(m_axial)
    ...
    wind_func = np.reshape(wind_func_computations, (1, m_axial, 1))
    ...
    reshaped_rf_data = rf_data.reshape(-1, m_axial)
    reshaped_wind_func = np.tile(wind_func_computations, (reshaped_rf_data.shape[0], 1))
    
    # Apply window function to all lines
    windowed_data = reshaped_rf_data * reshaped_wind_func
    
    # Compute FFT for each line
    fft_data = np.fft.fft(windowed_data, NUM_FOURIER_POINTS, axis=1) * reshaped_rf_data.shape[1]
    fft_magnitude = np.abs(fft_data) ** 2
    
    # Average power spectrum across all lateral and elevational lines
    full_ps = np.mean(fft_magnitude, axis=0)
